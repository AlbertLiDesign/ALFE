...
namespace ALFE.FESystem
{
    public class System3D
    {
        ...
        public System3D(Model3D model, bool unify = false)
        {
            Model = model;
            Unify = unify;
        ...
        public void Solve()
        {
            ...
            foreach (var item in Model.Nodes)
            {
                if (item.Active == true)
                {
                    item.Displacement = new Vector3D(X[id * DOF + 0], X[id * DOF + 1], X[id * DOF + 2]);
                    id++;
                }
            }
        }
        ...
        public float[,] GetDisplacement()
        {
            ...
            for (int i = 0; i < Model.Nodes.Count; i++)
            {
                ...
                displacement[i, 2] = Model.Nodes[i].Displacement.Z;
            }
            return displacement;
        }
